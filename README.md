...
| Key | Default | Comment in `analysis_params.yaml` | Range | What it changes |
|---|---|---|---|---|
| `processing.min_free_disk_gb` | 200 | minimum free GB on the working volume before extraction and before the build; 0 disables | number, 0 or more | Whether the driver refuses to begin a timepoint's frame extraction or Metashape build when the volume holding the processing folder has less free space than this. |

**`processing.min_free_disk_gb`.** Before each timepoint's frame extraction, and again before its Metashape build launches, the driver measures free space on the volume the processing folder lives on and refuses to start that piece of work when the free space is under this floor, failing the timepoint with a message that names the processing folder, the space left, and the floor. The check lives in the driver (`run_phase1.py`), so a bare `metashape -r step1.py` invocation does not perform it, and it does not re-run between the Metashape stages of a build already in flight. The check never waits, never prompts, and never interrupts a stage already in flight, so it cannot stall a hands-free run; the worst it does is stop the next expensive thing from starting on a drive that could not hold its output. 200 GB is sized to the module's own appetite: a 1000-frame timepoint writes about 212 GB of frames, and a two-chunk psx weighs 19 to 20 GB with depth maps deleted. This check is a failsafe, not the space manager. The planned sync driver is the primary mechanism for keeping the working volume clear, and when it exists this floor is simply the backstop for the window before the driver catches up. Set `0` to disable it entirely. It is separate from the temp-volume check (`STEP1_MIN_TEMP_FREE_GB`, 50 GB on `TMPDIR`), which guards where Metashape spills its pyramids rather than where the project grows.
...
The argv the runner builds, in `flag_map` order:
    ...
    python3 src/run_phase1.py [--input <in>] [--project <out>] --purpose "<purpose>" \
      ...
      [--transect <T#>] [--force] [--run-id <id>] [--params-version <ref>] \
      [--params-file <path>]

`--run-id`, `--params-version` and `--params-file` are the run identity flags the Voyager 1 setup window sends (module.yaml `cli.flag_map` keys `run_id_flag`, `params_version_flag` and `params_file_flag`). `--run-id` is the run id the setup window minted (for example `TCRMP_3sep26_LO_MRS3_23ann-25pbl`; letters, digits and `_ . + -`, at most 128 characters) and `--params-version` is the voyagerparams reference the parameters came from (for example `v1.2.0`, `branches/<slug>/v1.2.0` or `custom/<run id>`). Both are recorded for every selected registry row as `voyager1` facts (`run_id`, `params_version`) in the registry sidecar `vicarius/_METADATA/3d/row_facts.csv` the moment the processing folder is prepared, before step 0 runs, so the atlas drop-down can name the run and link its parameter version. `--params-file` names a YAML file (it must exist, be UTF-8 under 16 MB, and hold a top-level `processing:` mapping) that seeds a NEW processing folder's `analysis_params.yaml` in place of this module's template; a folder that already has the file keeps it and the run prints that the seed was ignored for that folder. `--param` overrides apply after the seed either way. A bad value for any of the three is refused by the parser (exit code 2, the reason on stderr) before Metashape is even looked for.

Step 0 and step 1 add their own `voyager1` facts to the same sidecar: while frames are being extracted the row's `console_log` cell points at the step 0 log (`console/step0_<TS>.log`) so the atlas can tail it, and a finished extraction records `step0_started`, `step0_finished` (both ISO with the -04:00 offset), `step0_seconds`, `frames_extracted` and `console_log_step0`; a finished reconstruction records `step1_report` (the PDF under `reports/`) and `console_log_step1`, both as links. Nothing is written outside TCRMP mode.
...
7. **Step 1 preflight.** Refuse to start when `TMPDIR` (or `/tmp`) has less than 50 GB free (`STEP1_MIN_TEMP_FREE_GB`), or when the processing folder's volume is under the `processing.min_free_disk_gb` floor (default 200 GB, 0 disables; the driver applies the same floor before each timepoint's frame extraction and again before its build launches). Then take an exclusive `flock` on `.processing.lock` stamped with pid, step, host, and start time. A second step against the same folder is refused with the holder printed. The driver holds this same lock from the moment the processing folder exists, through frame extraction, releasing it just before the build launches, so the whole run reads as live with no gap during extraction. While the lock is held the run counts as live: the atlas blocks edits and renames on the row, and a hard kill releases the lock so the row reads as interrupted instead.
...
- **A hard-killed run resumes on its own; `--force` is not the recovery.** A power cut, a `kill -9`, or an out-of-memory kill leaves the registry row with `stage` frozen at whatever was running and `step1_status` still `running`, but the OS releases the `.processing.lock` flock the moment the process dies, so the atlas shows the row as interrupted rather than running. Relaunch the same selection and the module picks each transect up from its latest stopping point: timepoints that finished stay finished, extracted frames are not re-extracted, and the interrupted timepoint runs again from where its record left off. The rerun appends `resumed after interrupted run` to the registry `notes` so the row's history says what happened. `--force` remains only for redoing work that actually finished.
- **A row is un-editable in the atlas while its run is live.** Rows whose runs have not started stay fully editable. The moment a run holds the folder's `.processing.lock`, the atlas refuses cell edits and `readable_id` renames on that row until the run finishes or dies; a hard-killed run releases the lock, gets the interrupted badge, and becomes editable again. Running versus interrupted is derived from `stage` plus the lock, never stored in a column, so the badge is always current even when the row's status cells are stale.
...
- **VICARIUS process logging now works with no export.** `run_phase1.py` and the platform logging library (`_logging/src/vicarius_log.py`) both default `VICARIUS_ROOT` to `/mnt/rip/vicarius_drive/vicarius`. Earlier builds of the library defaulted to a path that does not exist on this box, so every run hit a permission error creating the database directory and silently dropped its `process_start` and `process_end` events; the registry and `status.csv` were never affected. Export `VICARIUS_ROOT` only when the platform tree lives somewhere else.
...
- **`VICARIUS_ROOT` only needs exporting when the platform tree lives somewhere other than `/mnt/rip/vicarius_drive/vicarius`.** Both the module and the platform logging library default to that path, so on this box a bare-shell run writes its `process_start` and `process_end` events to the platform log with no environment setup.
...
A hard-killed run needs no cleanup either. The registry row keeps `stage` at its last value with `step1_status` still `running`, the lock file's flock is released by the OS, and the atlas badges the row interrupted. Relaunch the same selection: the module resumes each transect from its latest stopping point, appends `resumed after interrupted run` to the row's `notes`, and requires no `--force`.
...
- v1.1.0 (2026-08-31): Run guards and recovery. Processing folders are now created as a sibling of the folder holding the video, never inside it (corrected per LO). The driver holds the `.processing.lock` from folder preparation through frame extraction and step 1 holds it through the build, so a row's run state (running, interrupted, or idle) is derived from the lock at read time: the atlas blocks edits and renames only while a run is genuinely live, badges hard-killed runs as interrupted, and a plain relaunch resumes an interrupted timepoint from its latest stopping point with a `resumed after interrupted run` registry note and no `--force`. A stale `processing.force_rerun` left by a killed forced run is pinned false at the start of every ordinary timepoint. New `processing.min_free_disk_gb` floor (default 200, 0 disables) checked before each timepoint's extraction and again before its build; a too-full drive fails that row with a clear note and the run moves on. Metashape's long calls now log a progress percentage about every 30 seconds and every stage boundary logs a line, so the tailed console never goes quiet for a whole stage. Platform run events actually record again (the shared logging library pointed at a nonexistent root). Verified live: probed running during extraction and the build, rename refused, kill -9 mid-matching read interrupted within seconds, and the relaunch resumed and completed.
